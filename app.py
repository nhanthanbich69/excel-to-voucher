        ...
        for sheet_name in xls.sheet_names:
            ...
            for category in data_by_category:
                ...
                for mode in ["PT", "PC"]:
                    ...
                    out_df = pd.DataFrame()
                    out_df["Ngày hạch toán (*)"] = df_mode["NGÀY QUỸ"]
                    out_df["Ngày chứng từ (*)"] = df_mode["NGÀY KHÁM"]
                    ...
                    def gen_so_chung_tu(date_str):
                        try:
                            ...
                            return f"{mode}{date_str.replace("/", "")}_{chu_hau_to}"
                        except:
                            return f"{mode}_INVALID_{chu_hau_to}"

                    out_df["Số chứng từ (*)"] = df_mode["NGÀY KHÁM"].astype(str).apply(gen_so_chung_tu)
                    ...
                    out_df["Diễn giải lý do nộp"] = ("Thu tiền" if is_pt else "Chi tiền") + f" {noun} ngày " + df_mode["NGÀY KHÁM"].astype(str)
                        ...
                        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                            for mode in ["PT", "PC"]:
                                if mode in data and not data[mode].empty:
                                    data[mode].to_excel(writer, sheet_name=mode, index=False)
                        output.seek(0)
