        ...
        for sheet_name in xls.sheet_names:
            ...
            for category in data_by_category:
                ...
                for mode in ["PT", "PC"]:
                    ...
                    out_df["Diễn giải lý do nộp"] = ("Thu tiền" if is_pt else "Chi tiền") + " " + noun + " ngày " + out_df["Ngày chứng từ (*)"]
                        ...
                        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                            for mode in ["PT", "PC"]:
                                if mode in data and not data[mode].empty:
                                    df_full = data[mode]
                                    chunks = [df_full[i:i+500] for i in range(0, len(df_full), 500)]
                                    for idx, chunk in enumerate(chunks):
                                        sheet_title = mode if idx == 0 else f"{mode} {idx+1}"
                                        chunk.to_excel(writer, sheet_name=sheet_title, index=False)
                        output.seek(0)
